...
#define RSU_RANGE 200
#define RSU_RATE 10
#define VEHICLE_RANGE 100
...
int get_time_units(vector<float> speeds, float distance, vector<int> start);

int get_time_units(vector<float> speeds, float distance, vector<int> start) {
    int time_units=numeric_limits<int>::min();
    for(int i=0; i<speeds.size(); i++) {
        int current = start[i]+distance/speeds[i]+1;
        if(current > time_units) {
            time_units=current;
        }
    }
    return time_units;
...
void update_positions(vector<float> &positions, vector<float> speeds, vector<int> start, int cur_time, float distance) {
    for(int i=0; i<positions.size(); i++) {
        if(cur_time>=start[i] && positions[i]<distance)
            positions[i]+=speeds[i];
    }
}

void update_v2v_contacts(vector<float> speeds, float distance, vector<int> start, vector<vector<vector<bool>>> &v2v_contacts, int time_units) {
    vector<float> positions;
    for(int i=0; i<speeds.size(); i++) {
        positions.push_back(0);
    }
    long totaltime = 0;
    for(int k=0; k<time_units; k++) {
        for(int i=0; i<speeds.size(); i++) {
            for(int j=0; j<speeds.size(); j++) {
                if(i==j) {
                    v2v_contacts[i][j][k]=true;
                } else if(k<start[i] || k<start[j] || positions[i]>=distance || positions[j]>=distance) {
                    v2v_contacts[i][j][k]=false;
                } else if(abs(positions[i]-positions[j])>VEHICLE_RANGE) {
                    v2v_contacts[i][j][k]=false;
                } else {
                    v2v_contacts[i][j][k]=true;
                    totaltime++;
        ...
        update_positions(positions, speeds, start, k, distance);
        // for(int i=0; i<positions.size(); i++) {
        //     cout << positions[i] << ", ";
        // }
        // cout << endl;
    }
    //cout << "Total v2v contact duration: " << totaltime/2 << endl;
}

void update_v2r_contacts(vector<float> speeds, float distance, vector<int> start, vector<vector<vector<bool>>> &v2r_contacts, int time_units, vector<float> rsu) {
    vector<float> positions;
    for(int i=0; i<speeds.size(); i++) {
        positions.push_back(0);
    }
    for(int k=0; k<time_units; k++) {
        for(int i=0; i<speeds.size(); i++) {
            for(int j=0; j<rsu.size(); j++) {
                if(k<start[i] || positions[i]>=distance) {
                    v2r_contacts[i][j][k]=false;
                } else if(abs(positions[i]-rsu[j])>RSU_RANGE) {
                    v2r_contacts[i][j][k]=false;
                } else {
                    v2r_contacts[i][j][k]=true;
                }
            }
        }
        update_positions(positions, speeds, start, k, distance);
...
void updateStorage(vector<vector<vtransfer>> &vehicle_transfers, vector<vector<pair<int,int>>> &storage, int vehicle, int item, int datasize, int time) {
    ...
    if(storage[vehicle][item].second < 0) {
        cout << "Wtf " << storage[vehicle][item].second << endl;
    }
    ...
    while(!(vnew.empty())) {
        ...
        for(set<pair<int,int>>::iterator itr=affectedVehicles.begin(); itr!=affectedVehicles.end(); itr++) {
            ...
            if(abs(data_gained[v][d]-data_lost[v][d]) > RECURSION_CUTOFF*datasizes[d]) {
                ...
                //cout << "Current: " << current << " added: " << v << endl;
...
void cleanup(vector<vector<vtransfer>> &vehicle_transfers, map<int,unordered_set<int>> &vnew, vector<vector<int>> &requests, vector<vector<pair<int,int>>> &mainstorage, vector<vector<vector<bool>>> &v2v_contacts, vector<int> &datasizes, int time_units)
{
    while(!(vnew.empty())) {
        int current = vnew.begin()->first;
        unordered_set<int> items = vnew.begin()->second;
...
int main(void) {
    ...
    requests.resize(speeds.size(), vector<int>(1, 0));
    for(int i=0; i<speeds.size(); i++) {
        getline(inputfile, line);
        ss.clear();
        ss.str(line);
        int req, reqtime;
        while(ss >> reqtime) {
            //cout << "read" << reqtime << endl;
            requests[i][0]=reqtime;
        }
    }

    init_storage.resize(speeds.size(), vector<int>(1,0));
    getline(inputfile, line);
    cout << "read" << line << endl;
    ss.clear();
    ss.str(line);
    int vseed;
    while(ss >> vseed) {
        //init_storage[vseed][0] = 330;
    }

    time_units=get_time_units(speeds, distance, start);
    v2v_contacts.resize(speeds.size(), vector<vector<bool>>(speeds.size(), vector<bool>(time_units)));
    update_v2v_contacts(speeds, distance, start, v2v_contacts, time_units);
    // for(int k=0; k<time_units; k++) {
    //     for(int i=0; i<speeds.size(); i++) {
    //         for(int j=0; j<speeds.size(); j++) {
    //             cout << v2v_contacts[i][j][k] << " ";
    //         }
    //     }
    //     cout << endl;
    // }

    v2r_contacts.resize(speeds.size(), vector<vector<bool>>(rsu.size(), vector<bool>(time_units)));
    update_v2r_contacts(speeds, distance, start, v2r_contacts, time_units, rsu);
    // for(int k=0; k<time_units; k++) {
    //     for(int i=0; i<speeds.size(); i++) {
    //         for(int j=0; j<rsu.size(); j++) {
    //             cout << v2r_contacts[i][j][k] << " ";
    //         }
    //     }
    //     cout << endl;
    // }
    ...
    vehicle_transfers.resize(speeds.size(), vector<vtransfer>(time_units, vtransfer()));
    init_storage.resize(speeds.size(), vector<int>(datasizes.size(), 0));
    requests.resize(speeds.size(), vector<int>(datasizes.size(), INT_MAX));
    ...
    updateTransfers(vehicle_transfers, data_lost, data_gained, affectedVehicles, storage, requests, vnew, v2v_contacts, datasizes, 0, time_units);
        ...
        for(map<int,int>::iterator r=rsuCongestion.begin(); r!=rsuCongestion.end(); r++) {
            ...
            for(int slot=0; slot<RSU_MAX; slot++) {
                if(nextPeriod[j][slot]<=t) {
                    ...
                    cout << "T " << t << ": ";
                    for(int i=0; i<scores.size(); i++) {
                        cout << "(vehicle:" << scores[i].id << ", score: " << scores[i].score << "), ";
                    }
                    cout << endl;
                    ...
                    if(v_best.id == -1) {
                        ...
                        cout << "Updated by vehicle: " << v_best.id << endl;
                        ...
                        updateTransfers(vehicle_transfers, data_lost, data_gained, affectedVehicles, storage, requests, vnew, v2v_contacts, datasizes, t, time_units);
                    }
    ...
    cleanup(vehicle_transfers, vnew, requests, storage, v2v_contacts, datasizes, time_units);

    for(int t=0; t<time_units; t++) {
        for(int j=0; j<rsu.size(); j++) {
            cout << "RSU at time " << t << ": ";
            for(map<int,vehicle_score>::iterator itr = rsu_transfers[j][t].begin(); itr!=rsu_transfers[j][t].end(); itr++) {
                cout << "(vehicle:" << itr->second.id << ", score: " << itr->second.score << "), ";
            }
            cout << endl;
        }
    }
    ...
    for(int k=0; k<vehicle_transfers.size(); k++) {
        for(int item=0; item<datasizes.size(); item++) {
            ...
            cout << storage[k][item].second << endl;
    ...
    for(int t=0; t<time_units; t++) {
        int count = 0;
        vector<bool> recvehicles = vector<bool>(speeds.size(), false);
        for(int i=0; i<speeds.size(); i++) {
            for(int j=0; j<speeds.size(); j++) {
    ...
    cout << "V2V missed: " << v2v_miss << " percent: " << v2v_miss*100/v2vtot << endl;
    float sum = 0;
    for(int i=0; i<rsu.size(); i++) {
